  ...
  setup(props) {
    ...
    onMounted(async () => {
      ...
      activities.value = resp.activities.map(activity => ({
        time: new Date(Number(activity.replace('.jpg', ''))).toLocaleString(),
        image: `/image?cameraId=${selectedCamera.value.id}&activityId=${activity}&token=${authService.accessToken}`,
      }));
    });
